...
public class DownloadManager implements DTListener, PeerUpdateListener,
    ...
    // For displaying changes in the gui
     ...
     * @param clientID byte[]
     */
    public DownloadManager(TorrentFile torrent, final byte[] clientID, JTextArea log, JProgressBar jp, JLabel speedLabel) {
    ...
    public void setProgress(int percent) {
        if ((jp.getValue()!=percent) && (jp.getValue() <= 100))
            jp.setValue(percent);
    }
    ...
    public void blockUntilCompletion() {
        ...
        while (true) {
            ...
            setProgress((int)getCompleted());
            setSpeed((long)getDLRate());
            //log.append("\n Total: " + (long)getCompleted() + " % ******\n");
//            log.append(" Download Rate : " + (long)getDLRate() + " ko/s *******\n\n");
//            System.out.println(" Upload: " + getULRate());

            if (this.isComplete())
                log.append("\r\nSharing... Click on the Exit button to stop the client");
        }
        /*
    ...
    public void startTrackerUpdate() {
        this.pu = new PeerUpdater(this.clientID, this.torrent, log);
    ...
    public synchronized void pieceCompleted(String peerID, int i,
        ...
        synchronized (this.isComplete) {
            if (complete && !this.isPieceComplete(i)) {
                ...
                log.append("Piece completed by " + peerID +
                                   " : " + i + "\n");
                this.savePiece(i);
        ...
        synchronized (this.task) {
            ...
            for (Iterator it = l.iterator(); it.hasNext(); ) {
                ...
                if (p.getDLRate(false) > 0) {
                    dlrate += p.getDLRate(false)/(1024*10);
                    //System.out.println("UL rate: " +p.getULRate(false));
                    log.append(p + " rate: " +
